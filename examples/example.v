...
fn render_all_graph(ctx gg.Context, sdm rdm.Shear_and_moment_diagram, mvt rdm.Deplacements, stick rdm.Stick_type) {
	...
	mut value := []f32{len: nb, init: sdm.n.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'n en MPa')
	// ty
	y += h + dec
	value = []f32{len: nb, init: sdm.ty.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'ty en MPa')
	// mfz
	y += h + dec
	value = []f32{len: nb, init: sdm.mfz.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'mfz en MPa')
	y += h + dec
	value = []f32{len: nb, init: mvt.uy.value(l * index / nb)}
	...
	// right
	// mt
	value = []f32{len: nb, init: sdm.mt.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'mt en MPa')
	// tz
	y += h + dec
	value = []f32{len: nb, init: sdm.tz.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'tz en MPa')
	// mft
	y += h + dec
	value = []f32{len: nb, init: sdm.mfy.value(l * index / nb)}
	render_graph(ctx, x, y, w, h, abscise, value, 'mfy en MPa')
	y += h + dec
	value = []f32{len: nb, init: mvt.uz.value(l * index / nb)}
